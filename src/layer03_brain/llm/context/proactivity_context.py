...
async def build_proactivity_context() -> str:
    """Собирает контекст для проактивного цикла (с поддержкой Graph-RAG)"""
    ...
    try:
        ...
        unread_tg = _safe_get(results[7]) 
        macro_architecture_map = _safe_get(results[8])
        swarm_status = _safe_get(results[9])
        tasks = _safe_get(results[10])
        sandbox_files = _safe_get(results[11])


        # -----------------------------------------------------------------------------------
        # 2. GRAPH-RAG: фаза графа (ищем связи для активных задач и чатов)
        
        raw_anchors = _extract_anchors_for_proactivity(mental_state, tasks, unread_tg)
        anchors = [a for a in raw_anchors if a not in SUPERNODES]
        
        graph_context = "Нет активных связей в графе."
        associated_nodes = []
        
        if anchors:
            # Ограничим до 5 самых важных якорей, чтобы не перегружать граф
        ...
        # -----------------------------------------------------------------------------------
        # 3. GRAPH-RAG: фаза вектора (ассоциации)

        vector_queries = ["Анализ системы и планирование"]
        if "Список задач пуст" not in tasks:
            vector_queries.append(tasks[:200])

        if associated_nodes:
            system_logger.info(f"[Graph-RAG | Proactivity] Интуитивная связь с узлами: {associated_nodes}. Векторный поиск расширен.")
            vector_queries.extend(associated_nodes)

        # Единый запрос ко всем коллекциям
...
"""
        ...
        system_logger.error(f"Ошибка при формировании проактивного контекста: {e}")
        return "Критическая ошибка проактивного контекста."